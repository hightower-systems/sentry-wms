...
v1.4.5 -- Reverse-proxy hotfix follow-up. v1.4.4 wired Werkzeug `ProxyFix` behind `TRUST_PROXY` in the Flask app but did not add `TRUST_PROXY` to the `api` service environment in `docker-compose.yml`, so operators setting `TRUST_PROXY=true` in `.env` saw no effect: the value stopped at the Compose shell and never reached the container. Fruxh hit this after installing v1.4.4. v1.4.5 closes the gap: Compose forwards `TRUST_PROXY`, the Flask app logs the ProxyFix state at startup for `docker compose logs api | grep ProxyFix` verification, and `/api/health` now returns `proxy_fix_active` so the wiring is observable from outside the container. Deployment docs gain `.env`-location and `up -d` vs `restart` clarifications. 740 backend tests passing. See the [changelog](changelog.md), [SECURITY.md](https://github.com/hightower-systems/sentry-wms/blob/main/SECURITY.md), and the [v1.4.5 release](https://github.com/hightower-systems/sentry-wms/releases/tag/v1.4.5).
