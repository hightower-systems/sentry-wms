...
def _advance_so_to_picking(client, auth_headers, so_identifiers=None):
    ...
    idents = so_identifiers or ["SO-001"]
...
class TestLoadOrder:
    def test_load_order_for_packing(self, client, auth_headers):
        _advance_so_to_picking(client, auth_headers, ["SO-001"])
    ...
    def test_load_order_wrong_status(self, client, auth_headers):
        # SO-001 is still OPEN (not picked yet)
        resp = client.get("/api/packing/order/SO-001", headers=auth_headers)
...
class TestVerifyItem:
    def test_verify_item_success(self, client, auth_headers):
        ...
        # SO-001 line 1: item 1 (WIDGET-BLU), UPC 100000000001
        resp = client.post(
            "/api/packing/verify",
    ...
    def test_verify_over_pack(self, client, auth_headers):
        ...
        # SO-001 line 1 has quantity_picked = 2. Try to pack 100
        resp = client.post(
            "/api/packing/verify",
    ...
    def test_verify_item_not_on_order(self, client, auth_headers):
        ...
        # Item 3 (WIDGET-GRN, UPC 100000000003) is not on SO-001
        resp = client.post(
            "/api/packing/verify",
...
class TestCompletePacking:
    def _verify_all_items(self, client, auth_headers, so_id):
        """Verify all items on an order."""
        order_resp = client.get(f"/api/packing/order/SO-00{so_id}", headers=auth_headers)
    ...
    def test_calculated_weight_correct(self, client, auth_headers):
        ...
        resp = client.get("/api/packing/order/SO-001", headers=auth_headers)
        data = resp.get_json()
        # Calculate expected: item 1 (0.5 lbs * qty_picked) + item 6 (0.2 lbs * qty_picked)
        expected_weight = sum(
    ...
    def test_packing_requires_auth(self, client):
        resp = client.get("/api/packing/order/SO-001")
