...
- **Upgrade procedure:** `git pull && docker compose down && docker compose build && docker compose up -d`. The BUILD_VERSION guard (#73) catches skipped rebuilds. Five migrations (029-033) apply automatically on the next `db/seed.sh` invocation for fresh installs; existing deployments should run them via their usual migration runner in numeric order.
- **No mobile APK ships with v1.6.0.** Existing v1.4.1 / v1.4.3 APKs on Chainway C6000 devices continue to work; v1.6.0 has no mobile code changes beyond the version-string bump.
